 ...
 */
export const resolveAtTime = async (
  client: CeramicClient,
  id: StreamID,
  epoch: number,
 ...
 */
export const resolveVersionIndex = async (
  client: CeramicClient,
  id: StreamID,
  index: number,
): Promise<Stream> => {
...
/**
* Get the version history for a stream, excluding anchor commits.
*
* @param stream - The stream to get log from.
8 @returns The log of data commits.
*/
export const getVersionLog = (
  stream: Stream,
): Array<LogEntry & { commit: CommitID }> =>
  stream.state.log
    .filter((c) => c.type !== CommitType.ANCHOR)
