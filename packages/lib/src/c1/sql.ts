...
export const allResearchObjectsQuery = (model?: string) => `
  SELECT DISTINCT ON (stream_cid)
    controller,
    cid_string(stream_cid) as stream_cid,
    data::varchar->>'content' as state
  FROM event_states
  WHERE stream_id_string(arrow_cast(dimension_extract(dimensions, 'model'),'Binary')) = '${model ?? MODEL_IDS.researchObject}'
