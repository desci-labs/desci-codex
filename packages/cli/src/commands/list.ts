...
import { listNodes, getDriveTree, type DriveItem } from "../api.js";
import {
  createSpinner,
  printError,
  printTable,
...
function printTree(items: DriveItem[], prefix = ""): void {
  ...
  const cmd = new Command("list")
    ...
    .action(async (nodeArg: string | undefined, options) => {
      try {
        // Check API key
        if (!getApiKey()) {
          printError("No API key configured. Run: codex config login");
          process.exit(1);
        }
...
async function listAllNodes(options: { all?: boolean }): Promise<void> {
  ...
  try {
    ...
    if (nodes.length === 0) {
      ...
      console.log(chalk.dim("Create one with: codex push --new <folder>\n"));
      return;
    }
...
async function listNodeFiles(
  ...
  try {
    const tree = await getDriveTree(node.uuid);
    ...
    if (tree.length === 0) {
      ...
      console.log(
        chalk.dim(
          "Add files with: codex push <folder> --node " +
            node.uuid.slice(0, 8) +
            "\n",
        ),
      );
    ...
    if (options.tree) {
      ...
      // Simple list
      const listFiles = (items: DriveItem[], path = ""): void => {
