...
function countFiles(items: DriveItem[]): number {
  let count = 0;
...
function findItemByPath(
  ...
  let current: DriveItem[] = items;
  let found: DriveItem | null = null;
...
export function createPullCommand(): Command {
  return new Command("pull")
    ...
    .option("--published", "Pull from published version (not draft)")
    .action(async (nodeArg: string | undefined, options) => {
      try {
        ...
        if (!targetUuid) {
          ...
          if (nodes.length === 0) {
            printError(
              "No nodes found. Create one first with: codex push --new",
            );
            process.exit(1);
          }
        ...
        let tree: DriveItem[];
        try {
          tree = await getDriveTree(targetUuid);
