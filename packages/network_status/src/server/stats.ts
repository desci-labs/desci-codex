...
export const getNetworkStats = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const statsQuery = `
      SELECT 
        (SELECT COUNT(*) FROM nodes WHERE environment = $2) as total_nodes,
        (SELECT COUNT(*) FROM nodes WHERE last_seen_at >= $1 AND environment = $2) as active_nodes,
        (SELECT COUNT(*) FROM manifests WHERE environment = $2) as total_manifests,
